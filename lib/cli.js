...
class Cli {
  ...
  parseLogTimeArgs(args) {
    ...
    args.forEach(arg => {
      ...
      } else if (m = arg.match(/^(?<hours>\d{0,2}):(?<minutes>\d{2})$/)) {
        timedef.hours = ((+m.groups.hours * 60 + +m.groups.minutes) / 60).toFixed(2);
  ...
  logTimeCli(args) {
    ...
    this.logTime(timedef.issue_id, timedef.hours, timedef.activity_id, timedef.spent_on);
  }
