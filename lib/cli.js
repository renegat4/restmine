...
class Cli {
  ...
  getTicket(issueId) {
    return this.api
      .getTicket(issueId)
      .then(_ => JSON.parse(_).issue)
      .then(_ => this.printTicket(_))
  ...
  takeOver(handle, issue) {
    ...
    // TODO Den Status nur ändern wenn er auf 'Neu' steht.
    if (issue.status.id != editId) {
      data.issue.status_id = editId;
      update = true;
    }

    // TODO Die Kategorie nur ändern wenn noch keine gesetzt ist.
   ...
   */
  enterIssueBranch(issue) {
    return this.api
      .getTicket(issue.id)
      .then(_ => JSON.parse(_).issue)
      .then(_ => this.printTicket(_))
      .then(_ => this.takeOver(issue, _));
  }
  ...
  getActivityId(issueType) {
    const type = `${issueType.toLowerCase()}_activityid`;
    if (this.config[type]) {
      return this.config[type];
    } else {
      throw new Error(`Issue-Typ ${type} nicht gesetzt`);
    }
  }
