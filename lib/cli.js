...
class Cli {
  ...
  tableSpecs(issues) {
    ...
    for (const i of a) {
      result.push(
        issues
          .map(i)
          .reduce((accu, str) => str.length > accu ? str.length : accu, 0)
      );
    }
    return result;
  }
  ...
  tabular(issue, specs) {
    const output = [];
    output.push(this.pad(issue.id.toString(), specs[0]));
    output.push(this.pad(issue.priority.name, specs[1]));
    output.push(this.pad(issue.tracker.name, specs[2]));
    output.push(this.pad(issue.status.name, specs[3]));
