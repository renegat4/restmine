...
class Cli {
  ...
  formatProjects(list) {
    ...
    for (let p of list.sort((a, b) => a.name.localeCompare(b.name))) {
      line = `${p.id}\t${p.name}`;
      line += padding.substr(p.name.length, maxlen);
      if (p.description) {
  ...
  printTicket(issue, tabular) {
    if (tabular) {
      this.print(this.tabular(issue));
    } else {
  ...
  tabular(issue) {
    const output = [];
    output.push(this.pad(issue.id, 6));
    output.push(this.pad(issue.priority.name, 8));
    output.push(this.pad(issue.tracker.name, 10));
    output.push(this.pad(issue.status.name, 12));
    output.push(colors.green(issue.subject));
    return output.join(' ');
  }
