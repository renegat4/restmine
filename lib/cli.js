...
class Cli {
  ...
  formatProjects(list) {
    ...
    const padding = new Array(maxlen).fill(' ').join('');
  ...
  tabular(issue) {
    const output = [];
    output.push(issue.id);
    output.push(issue.priority.name);
    output.push(issue.tracker.name);
    output.push(issue.status.name);
